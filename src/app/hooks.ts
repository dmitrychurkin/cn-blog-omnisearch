...
export const useRedirect = () => {
    ...
    const getBaseUri = useCallback((suggestionType?: SuggestionTypeEnum) => {
        ...
        if (typeof baseUri === 'string') {
            ...
            if ((typeof seletedSuggestion?.slug === 'string') &&
    ...
    const getRedirectLink = useCallback((searchQueryMap: Map<SearchQueryParamsType, string>, suggestionType?: SuggestionTypeEnum) => {
        ...
        searchQueryMap.forEach((value: string, key: string) => {
            if (suggestionType && restrictedParams.get(suggestionType)?.includes(key)) {
                return;
            }
    ...
    const getSearchQueryParams = useCallback((additionalSearchQueryParams?: Map<SearchQueryParamsType, string>) => {
        const {
            location: { location, currentLocationEntity: { city, country } },
            date: { checkIn, checkOut },
            guest: { adult, child, infant }
        } = store.getState();

        const locationString = location.trim() || constructLocationString({ city, country });
    ...
    const generateLinkStrategies = useMemo(() =>
        new Map<SuggestionTypeEnum, (strategy: SuggestionTypeEnum) => string | undefined>()
            .set(SuggestionTypeEnum.NEARBY, (strategy: SuggestionTypeEnum) => {
                const {
                    location: { currentLocationEntity: { city, country } },
                } = store.getState();
            ...
            .set(SuggestionTypeEnum.DEFAULT, (strategy: SuggestionTypeEnum) => getRedirectLink(
                getSearchQueryParams(),
                strategy
            ))
    , [
    ...
    return useCallback((strategy: SuggestionTypeEnum) => {
        const linkGeneratedStrategy = generateLinkStrategies.get(strategy) || generateLinkStrategies.get(SuggestionTypeEnum.DEFAULT);
...
type SearchQueryParamsType = 'location' | 'checkin' | 'checkout' | 'adults' | 'children' | 'infants' | 'rooms';
