...
const GuestSelector: React.FC<Prop> = ({
    ...
    const [guestState, handleChange, constraint] = useGuestSelect(guestSelectorState);

    const onCountChange = useCallback((guestType: GuestTypeEnum) =>
        (op: OperationTypeEnum, count: number) => {
        ...
        return (
            ...
            >
                <header className={styles.header}>
                    <Button
                        className={clsx(
                            'omnisearch-guest-select_close-btn',
                            styles.close
                        )}
