...
const SuggestMenu: React.FC<Prop> = ({ locationInput, suggestions, mode, onSelect }) => {
    const defaultGlobalClassName = 'omnisearch-suggest-menu';

    const getSuggestUnit = useCallback((customSuggestionType?: SuggestionTypeEnum) => ({ id, type, name, country, state, city }: SuggestionType, index: number) => (
        <SuggestUnit
            key={id ?? String(index)}
