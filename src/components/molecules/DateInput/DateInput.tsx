...
const DateInput: React.FC<Prop> = ({ startIcon: StartIcon, startDate, endDate, onDatesChange, focusedInput, onFocusChange }) => {
    ...
    const onClose = useCallback(({ startDate, endDate }: IDateInputPayload) => {
        if (startDate && !endDate) {
            onDatesChange({ startDate, endDate: moment(startDate).add(1, 'day') });
        } else if (!startDate && endDate) {
            onDatesChange({ startDate: moment(endDate).subtract(1, 'day'), endDate });
        }
    }, [onDatesChange]);
        ...
        <>
            <Button
                className={clsx(
                    ...
                    (focusedInput !== null) && ['omnisearch-date-input--focus', styles.focus],
                    Boolean(startDate || endDate) && ['omnisearch-date-input--active', styles.active],
                    styles.root
                )}
                startIcon={StartIcon}
