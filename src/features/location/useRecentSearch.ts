...
export default function useRecentSearch() {
    ...
    const updateRecentSearch = useCallback((data?: Partial<SuggestPayloadType> & Partial<IDateBase> & Partial<IGuestBase> & { strategyType: SuggestionTypeEnum | undefined }, lsKey = 'recentSearch') => {
        const { location: { seletedSuggestion: ss, currentLocationEntity }, date, guest } = store.getState();
        const dataToSet = {
            id: ss?.id,
            type: ss?.type,
            slug: ss?.slug,
        ...
        const seletedSuggestion = (data?.strategyType === SuggestionTypeEnum.NEARBY) ||
            (dataToSet.type === SuggestionTypeEnum.NEARBY)
            ? { city, country } : { name, city, state, country };
        ...
        const filteredState = getRecentSearch().filter(it => {
            ...
            const formattedLocationArray = it
                ?.addrData
                ?.formattedAddress
                .split(',')
            ...
            if (localCity === (seletedSuggestion.city || seletedSuggestion.name) && localCountry === seletedSuggestion.country) {
                return false;
            }

            const currentLocationArray = constructLocationString(seletedSuggestion, ',').split(',');

            return JSON.stringify(formattedLocationArray) !== JSON.stringify(currentLocationArray);
        });
        ...
        const updatedState = [
            {
                activeTab: 'home',
                addrData: {
                    address: (type !== SuggestionTypeEnum.COUNTRY) ? constructLocationString({ city, state, country }) : '',
                    formattedAddress: constructLocationString((type === SuggestionTypeEnum.NEARBY) ? { city, country } : seletedSuggestion),
                    id,
                    name,
                    slug,
                    type
